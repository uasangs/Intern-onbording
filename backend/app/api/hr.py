...
from app.services.file_service import save_upload, read_upload
...
def initiate_intern(
    ...
    # AccountsTask is NOT created here — it is created only when candidate accepts the offer letter

    # ITTask is NOT created here — it is created only when candidate accepts the offer letter
...
def get_intern(
    ...
    return InternRecordOut.from_orm(record)
...
# ── Download a candidate document (HR) ───────────────────────────────────────
...
def download_document(
    ...
    if not doc.file_data:
        raise HTTPException(status_code=404, detail="No file data available for this document")
...
# ── Download offer letter (HR) ────────────────────────────────────────────────

@router.get("/offer-letter/{intern_id}/download")
def download_offer_letter(
    ...
    if not offer or not offer.file_data:
        raise HTTPException(status_code=404, detail="Offer letter not found or not yet generated")
    file_name = offer.file_name or "offer_letter.pdf"
    return Response(
...
def verify_document(
    ...
    db.commit()
    db.refresh(doc)
    action = "DOCUMENT_APPROVED" if payload.status == "approved" else "DOCUMENT_REJECTED"
    log_action(db, str(doc.candidate.intern_record.id), str(current_user.id), action,
               entity_type="Document", entity_id=str(doc.id),
               field_changed="status", new_value=str(payload.status),
               old_value=payload.rejection_reason or "")
    return doc
...
async def generate_offer(
    ...
    offer.file_data = pdf_bytes
    offer.file_name = f"offer_letter_{record.candidate_email}.pdf"
    offer.content_type = "application/pdf"
    offer.pdf_url = None
...
async def send_offer(
    ...
    if not record.offer_letter or not record.offer_letter.file_data:
        ...
        offer.file_data = pdf_url  # pdf_url now holds bytes from pdf_service
        offer.file_name = f"offer_letter_{record.candidate_email}.pdf"
        offer.content_type = "application/pdf"
        offer.pdf_url = None
        offer.is_hr_uploaded = False
    ...
    log_action(db, str(record.id), str(current_user.id), "OFFER_LETTER_SENT",
               entity_type="OfferLetter",
               new_value=record.candidate_email)
...
async def generate_certificate(
    ...
    pdf_bytes = await generate_certificate_pdf(record, payload)
    ...
    cert.file_name = f"certificate_{record.candidate_email}.pdf"
    cert.content_type = "application/pdf"
    cert.pdf_url = None
    ...
    db.commit()
    db.refresh(cert)
    log_action(db, str(record.id), str(current_user.id), "CERTIFICATE_GENERATED",
               entity_type="ExperienceCertificate", entity_id=str(cert.id),
               new_value=payload.project_title)
    return cert
...
# ── Download certificate (HR) ─────────────────────────────────────────────────

@router.get("/certificate/{intern_id}/download")
def download_certificate(
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Intern record not found")
    cert = record.experience_certificate
    if not cert or not cert.file_data:
...
def send_certificate_to_candidate(
    ...
    if not record.experience_certificate.file_data:
...
# ── HR edits candidate pre-fill data ─────────────────────────────────────────

@router.patch("/intern/{intern_id}/candidate-prefill")
def update_candidate_prefill(
    intern_id: UUID,
    payload: dict,
    db: Session = Depends(get_db),
    current_user: HRUser = Depends(require_hr),
    ...
    c = record.candidate
    if "candidate_name" in payload: c.full_name = payload["candidate_name"] or c.full_name
    if "candidate_gender" in payload: c.gender = payload["candidate_gender"] or c.gender
    if "candidate_mobile" in payload: c.mobile = payload["candidate_mobile"] or c.mobile
    if "candidate_city" in payload: c.city = payload["candidate_city"] or c.city
    if "candidate_state" in payload: c.state = payload["candidate_state"] or c.state
    if "institute_name" in payload: c.institute_name = payload["institute_name"] or c.institute_name
    if "qualification" in payload: c.qualification = payload["qualification"] or c.qualification
    if "course" in payload: c.course = payload["course"] or c.course
    if "year_of_study" in payload: c.year_of_study = payload["year_of_study"] or c.year_of_study
    if "graduation_year" in payload and payload["graduation_year"]: c.graduation_year = int(payload["graduation_year"])
    db.commit()
    log_action(db, str(record.id), str(current_user.id), "CANDIDATE_PREFILL_UPDATED",
               entity_type="Candidate", entity_id=str(c.id))
    return {"message": "Candidate information updated. Please resend the portal link to the candidate."}


# ── Enable intern self-review ────────────────────────────────────────────────

@router.post("/intern/{intern_id}/enable-self-review")
def enable_self_review(
    intern_id: UUID,
    db: Session = Depends(get_db),
    current_user: HRUser = Depends(require_hr),
):
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Not found")
    record.self_review_enabled = True
    db.commit()
    log_action(db, str(record.id), str(current_user.id), "SELF_REVIEW_ENABLED",
               entity_type="InternRecord")
    return {"message": "Self-review enabled for candidate"}
