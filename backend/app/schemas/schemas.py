...
class InternRecordOut(BaseModel):
    ...
    def from_orm(cls, obj):
        ...
        if obj.offer_letter:
            instance.offer_letter = OfferLetterOut.from_orm(obj.offer_letter)
        if obj.experience_certificate:
            instance.experience_certificate = CertificateOut.from_orm(obj.experience_certificate)
        return instance
...
class DocumentOut(BaseModel):
    ...
    file_data_available: bool = False
    status: DocStatus
    rejection_reason: Optional[str] = None
    uploaded_at: datetime
    _file_data: Optional[bytes] = None  # Internal field to carry raw bytes

    class Config:
        from_attributes = True

    @validator('file_data_available', always=True, pre=False)
    @classmethod
    def set_file_data_available(cls, v, values):
        # This validator runs after field assignment but can't access ORM object
        # We rely on the explicit field below instead
        return v
    ...
    def from_orm(cls, obj):
        d = cls(
            ...
            file_data_available=obj.file_data is not None,
            status=obj.status,
...
class OfferLetterOut(BaseModel):
    ...
    @validator('has_file', always=True, pre=True)
    @classmethod
    def compute_has_file(cls, v, values, **kwargs):
        return v  # Will be overridden by from_orm
    ...
    def from_orm(cls, obj):
        instance = cls(
            ...
            pdf_url=obj.pdf_url,
            file_name=obj.file_name,
            has_file=obj.file_data is not None,
            content_type=obj.content_type,
            is_hr_uploaded=obj.is_hr_uploaded or False,
            status=obj.status,
            sent_at=obj.sent_at,
            responded_at=obj.responded_at,
            candidate_response=obj.candidate_response,
            candidate_remarks=obj.candidate_remarks,
        )
        return instance
...
class CertificateOut(BaseModel):
    ...
    def from_orm(cls, obj):
        instance = cls(
            ...
            guide_names=obj.guide_names,
            conduct_remark=obj.conduct_remark,
            issue_date=obj.issue_date,
            delivered_to_candidate=obj.delivered_to_candidate or False,
...
class PortalInfo(BaseModel):
    ...
    account_holder_name: Optional[str] = None
    account_type: Optional[str] = None
    # Status flags for portal flow control
    self_review_submitted: bool = False
    self_review_enabled: bool = False
    # Which fields HR pre-filled (so portal can lock them)
    hr_prefilled_fields: List[str] = []
