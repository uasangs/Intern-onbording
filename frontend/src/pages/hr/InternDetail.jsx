...
//                 <a
//                   href={fileDownloadUrl(offer.pdf_url)}
//                   download
...
//             </>
//           )}
//           {offer?.pdf_url && offer.status !== 'sent' && offer.status !== 'accepted' && (
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {offer?.pdf_url && (
            <a
              ...
              className="btn-secondary flex items-center gap-2"
            >
              <FileText className="w-4 h-4" />Download Offer Letter {offer.pdf_url.endsWith('.docx') ? '(.docx)' : '(.pdf)'}
            </a>
          )}
          {offer?.pdf_url && offer.status !== 'sent' && offer.status !== 'accepted' && (
            <button onClick={sendOffer} className="btn-primary flex items-center gap-2">
          ...
          {cert?.pdf_url ? (
            <div className="space-y-4">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {cert.pdf_url && (
                  <a href={fileDownloadUrl(cert.pdf_url)} download className="btn-secondary flex items-center gap-2 text-xs">
                    <FileText className="w-3.5 h-3.5" />Download {cert.pdf_url.endsWith('.docx') ? '(.docx)' : '(.pdf)'}
                  </a>
                )}
              </div>
              {/* Send certificate */}
